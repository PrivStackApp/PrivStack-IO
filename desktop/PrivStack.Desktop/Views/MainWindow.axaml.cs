...
public partial class MainWindow : Window
{
    ...
    private void OnWindowOpened(object? sender, EventArgs e)
    {
        ...
        // Adjust title bar spacer and content margin to actual platform decoration height
        if (decorTop > 0 && Math.Abs(decorTop - 28) > 0.5)
        {
            Log.Information("[MainWindow] Adjusting title bar offset from 28 to {DecorTop}", decorTop);
            TitleBarSpacer.Height = decorTop;
            var rootPanel = this.Content as Panel;
            var mainGrid = rootPanel?.Children.OfType<Avalonia.Controls.Grid>().FirstOrDefault();
            if (mainGrid != null)
                mainGrid.Margin = new Thickness(0, decorTop, 0, 0);
        }
